...
class App:
    ...
    def login_admin(self):
        ...
        id_admin = input(Colores.VERDE + 'ID: ' + Colores.RESET).strip()
        clave_admin = input(Colores.VERDE + 'CLAVE: ' + Colores.RESET).strip()
        if (Administrador(ad.ID, ad.CLAVE) ==
                Administrador(id_admin, clave_admin)):
            print(Colores.VERDE_CLARITO + 'Autentificación correcta.' + Colores.RESET)
            print(App.SEPARADOR)
    ...
    def login_profesor(self):
        ...
        id_prof = input(Colores.MAGENTA + 'ID: ' + Colores.RESET)
        clave_prof = input(Colores.MAGENTA + 'CLAVE: ' + Colores.RESET)
        if BaseDatos.autentificar_profesor(id_prof, clave_prof):
            print(Colores.CIAN + 'Autentificación correcta.' + Colores.RESET)
        ...
        match opcion.strip().lower():
            case Menus.ConstantesMenu.UNO:
                ...
                if BaseDatos.vaciar_bbdd():
                    print(Colores.ROJO + ('La base de datos se vació '
                                        'correctamente.'))
    ...
    def comprobar_resultado_backup(self, resultados: tuple[bool, str]):
        print(App.SEPARADOR)
        if resultados[0]:
            print(f'{Colores.ROJO}La copia de seguridad se guardo correctamente, '
    ...
    def dar_de_alta_guardia(self, *args):
        ...
        if guardia.insertar_guardia():
            print(Colores.ROSA + 'Insercción ejecutada correctamente.' +
                  Colores.RESET)
            self.acabar_operacion()
        else:
            self.gestionar_error_bbdd()
    ...
    def generar_informe(self):
        print('Para ver el informe de las guardias debes introducir '
              ...
              'Para la FECHA DE INICIO:' + Colores.RESET )
        fecha_inicio = self.generar_fecha()
        print(Colores.ROJO + 'Para la FECHA FINAL:'+ Colores.RESET)
    ...
    def elegir_objeto_de_lista(self, lista):
        ...
        if lista:
            ...
            print(Colores.ROJO + 'No hay opciones disponibles.' + Colores.RESET)
    ...
    def generar_fecha(self) -> date:
        try:
            ...
            if date.weekday(fecha) not in App.FINES_SEMANA:
                ...
                print(App.SEPARADOR)
                print(f'{Colores.ROJO}La fecha introducida se corresponde a un '
                      f'día no lectivo.{Colores.RESET}Si desea salir pulsa '
    ...
    def gestionar_fallo_autentificacion(self):
        print(App.SEPARADOR)
        print(f'{Colores.ROJO}Usuario y/o clave incorrectas. {Colores.RESET}Si '
              f'quieres intentar de nuevo introduce {Menus.ConstantesMenu.SEGUIR}, si quieres salir '
    ...
    def gestionar_error_bbdd(self):
        print(f"{Colores.ROJO}Ha ocurrido un error relacionado con la Base de "
              f"Datos. {Colores.RESET}")
    ...
    def acabar_operacion(self):
        print(App.SEPARADOR)
        print(f'{Colores.ROSA}Operación ejecutada correctamente.{Colores.RESET}\n'
    ...
    @staticmethod
    def imprimir_guardias(guardias: list[Guardia]):
        if guardias:
            for guardia in guardias:
                print(App.SEPARADOR)
                print(guardia)
        else:
            print(Colores.ROJO + ('No se encontró ninguna guardia para estas '
                                'fechas.' + Colores.RESET))
